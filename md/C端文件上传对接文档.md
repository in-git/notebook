...
const uploadSingle = async (file) => {
  ...
  fd.append('file', file);
  const data = await clientBaseRequest('/client/c/fileFolder/upload', fd, 'post', {
...
const uploadBatch = async (fileList) => {
  ...
  const data = await clientBaseRequest('/client/c/fileFolder/uploadBatch', fd, 'post', {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
  return data; // [{ id, name, url, ... }, ...]
};
...
返回 DTO 引用 [DevFileFolderUploadResult.java](file:///e:/git/bkend/snowy-plugin/snowy-plugin-biz/src/main/java/vip/xiaonuo/biz/modular/fileFolder/param/DevFileFolderUploadResult.java)，前端可按字段定义 TS 接口。
